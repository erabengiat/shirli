...
function cleanHtml(res){
  ...
  return res.text().then(html=>{
    const css=`<style id="shirli-v47-css">
    ...
    if(!html.includes('shirli-v47-css')) html=html.replace('</head>',css+'\n</head>');
    ...
    const js=`<script id="shirli-v47-js">(function(){
      ...
      function fixSlideshow(){
        window.renderSlide=function(){let c=(window.slideOrder||[])[window.slideIdx||0],body=document.getElementById('slideBody');if(!body)return;if(!c){body.innerHTML='';return}let src=c.img||('images/'+c.no+'.jpg');let img=document.createElement('img');img.className='slide-img';img.alt='';img.onload=()=>{};img.onerror=()=>{let fallback='images/'+c.no+'.jpg';if(img.getAttribute('data-fallback')!=='1'&&img.src.indexOf(fallback)===-1){img.setAttribute('data-fallback','1');img.src=fallback}else{body.innerHTML='<div style="color:#fff;font-size:14px">לא ניתן לטעון את התמונה</div>'}};body.innerHTML='';body.appendChild(img);img.src=src};
      }
    ...
    if(!html.includes('shirli-v47-js')) html=html.replace('</body>',js+'\n</body>');
